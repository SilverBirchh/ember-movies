...
module('Unit | Service | movies', function (hooks) {
  ...
  test('can like a movie and save it to storage', async function (assert) {
    ...
    set(service, 'movies', {
      results: [{
        id: 1, title: 'Robo Cop', release_date: '2019-09-08', poster_path: '/path'
      }]
    });
  ...
  test('can fetch cached movie by id', async function (assert) {
    let service = this.owner.lookup('service:movies');
    set(service, 'movies', {
      results: [{
        id: 1, title: 'Robo Cop', release_date: '2019-09-08', poster_path: '/path'
    ...
    const movie = await service.getMoviesById(1);
    assert.equal(movie.title, 'Robo Cop');
  });

  test('can fetch movie over the network', async function (assert) {
    const authFetch = sinon.stub().resolves({
      results: [{
        id: 1, title: 'Robo Cop', release_date: '2019-09-08', poster_path: '/path'
      }]
  ...
  test('can fetch a movie that is not in recent movie list', async function (assert) {
    const authFetch = sinon.stub()
    authFetch.onFirstCall().resolves({
      results: [{
        id: 1, title: 'Robo Cop', release_date: '2019-09-08', poster_path: '/path'
      }]
    });
    authFetch.onSecondCall().resolves({
      id: 2, title: 'Back to the Future', release_date: '2019-09-08', poster_path: '/path'
    ...
    const movie = await service.getMoviesById(2);
